...
function warnCustomizedOption(config, key, defaultValue, customMessage, configFileName, silent) {
    ...
    if (!silent && current !== defaultValue) {
        _log.warn(`The "${key}" option has been modified. ${customMessage ? customMessage + '. ' : ''}Please update your ${configFileName}.`);
    }
}
function assignDefaults(dir, userConfig, silent) {
    var _defaultConfig_experimental, _defaultConfig_experimental1, _result_experimental, _result_experimental1, _result_experimental2, _result_experimental_serverActions, _result_experimental3, _result_devIndicators, _result_experimental4;
    ...
    if ((_defaultConfig_experimental1 = _configshared.defaultConfig.experimental) == null ? void 0 : _defaultConfig_experimental1.pprFallbacks) {
        _log.warn(`\`experimental.pprFallbacks\` has been defaulted to \`true\` because \`__NEXT_EXPERIMENTAL_PPR\` was set to \`true\` during testing.`);
    }
    const result = {
        ..._configshared.defaultConfig,
        ...config
    };
    if (((_result_experimental = result.experimental) == null ? void 0 : _result_experimental.pprFallbacks) && !((_result_experimental1 = result.experimental) == null ? void 0 : _result_experimental1.ppr)) {
        throw new Error(`The experimental.pprFallbacks option requires experimental.ppr to be set to \`true\` or \`"incremental"\`.`);
    }
    if (((_result_experimental2 = result.experimental) == null ? void 0 : _result_experimental2.allowDevelopmentBuild) && process.env.NODE_ENV !== 'development') {
            ...
            if ((_config_assetPrefix = config.assetPrefix) == null ? void 0 : _config_assetPrefix.startsWith('http')) {
                try {
                    ...
                    if (!hasMatchForAssetPrefix) {
                        ...
                        (_images_remotePatterns = images.remotePatterns) == null ? void 0 : _images_remotePatterns.push({
    ...
    warnOptionHasBeenDeprecated(result, 'experimental.instrumentationHook', '`experimental.instrumentationHook` is no longer needed to be configured in Next.js', silent);
    ...
    warnOptionHasBeenMovedOutOfExperimental(result, 'swrDelta', 'swrDelta', configFileName, silent);
    ...
    if (typeof ((_result_experimental3 = result.experimental) == null ? void 0 : (_result_experimental_serverActions = _result_experimental3.serverActions) == null ? void 0 : _result_experimental_serverActions.bodySizeLimit) !== 'undefined') {
    ...
    if ((result == null ? void 0 : result.outputFileTracingRoot) && !(0, _path.isAbsolute)(result.outputFileTracingRoot)) {
        ...
        if (!silent) {
            _log.warn(`experimental.outputFileTracingRoot should be absolute, using: ${result.outputFileTracingRoot}`);
        }
    }
    ...
    if (path == null ? void 0 : path.length) {
        ...
        try {
            ...
            if (process.env.__NEXT_TEST_MODE === 'jest') {
                ...
                curLog.warn(`Configuration with ${configFileName} is currently an experimental feature, use with caution.`);
            } else {
