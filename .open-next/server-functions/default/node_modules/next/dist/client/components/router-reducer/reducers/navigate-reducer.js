...
function navigateReducer(state, action) {
    ...
    return data.then((param)=>{
        ...
        // data is loaded.
        const updatedCanonicalUrl = canonicalUrlOverride && !prefetchValues.aliased ? (0, _createhreffromurl.createHrefFromUrl)(canonicalUrlOverride) : href;
        // Track if the navigation was only an update to the hash fragment
        mutable.onlyHashChange = !!hash && state.canonicalUrl.split('#', 1)[0] === updatedCanonicalUrl.split('#', 1)[0];
        let currentTree = state.tree;
        let currentCache = state.cache;
        let scrollableSegments = [];
        for (const normalizedFlightData of flightData){
            const { tree: treePatch, pathToSegment: flightSegmentPath, seedData, head, isRootRender } = normalizedFlightData;
            // TODO-APP: remove ''
            ...
            if (newTree !== null) {
                ...
                seedData && isRootRender && postponed) {
                    const task = (0, _pprnavigations.updateCacheNodeOnNavigation)(currentCache, currentTree, treePatch, seedData, head, mutable.onlyHashChange);
                    if (task !== null) {
                    ...
                    if (prefetchValues.aliased && seedData) {
                        if (isRootRender) {
                            ...
                            (0, _fillcachewithnewsubtreedata.fillCacheWithNewSubTreeDataButOnlyLoading)(cache, currentCache, normalizedFlightData, prefetchValues);
                        }
                        applied = true;
                    } else if (prefetchValues.status === _routerreducertypes.PrefetchCacheEntryStatus.stale && !mutable.onlyHashChange && !isFirstRead) {
