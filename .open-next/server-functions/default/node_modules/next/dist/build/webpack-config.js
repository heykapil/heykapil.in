...
if (parseInt(_react.default.version) < 19) {
...
async function getBaseWebpackConfig(dir, { buildId, encryptionKey, config, compilerType, dev = false, entrypoints, isDevFallback = false, pagesDir, reactProductionProfiling = false, rewrites, originalRewrites, originalRedirects, runWebpackSpan, appDir, middlewareMatchers, jsConfig, resolvedBaseUrl, supportedBrowsers, clientRouterFilters, fetchCacheKeyPrefix, edgePreviewProps }) {
    var _config_experimental, _config_experimental1, _config_experimental_sri, _config_experimental_sri1, _config_compiler, _config_compiler1, _config_compiler2, _jsConfig_compilerOptions, _config_compiler3, _jsConfig_compilerOptions1, _config_compiler4, _jsConfig_compilerOptions2, // always add JsConfigPathsPlugin to allow hot-reloading
    // if the config is added/removed
    _webpackConfig_resolve_plugins, _webpackConfig_resolve, _config_experimental2, _config_compiler5, _config_compiler6, _config_compiler7, _config_compiler8, _config_compiler9, _webpackConfig_module, _webpackConfig_module1, _webpackConfig_module_rules, _webpackConfig_module2;
    ...
    const isFullFlyingShuttle = (flyingShuttle == null ? void 0 : flyingShuttle.mode) === 'full';
    ...
    let webpackConfig = {
            ...
            ...isFullFlyingShuttle ? {
                ...
                filename: (p)=>{
                    if (isNodeOrEdgeCompilation) {
                        var _p_chunk_name, _p_chunk;
                        // runtime chunk needs hash so it can be isolated
                        // across builds
                        const isRuntimeChunk = (_p_chunk = p.chunk) == null ? void 0 : (_p_chunk_name = _p_chunk.name) == null ? void 0 : _p_chunk_name.match(/webpack-(api-runtime|runtime)/);
                        return `${isEdgeServer ? '' : '../'}[name]${isRuntimeChunk ? `-${buildId}` : ''}.js`;
                    }
                    // client filename
                    return `static/chunks/[name]-[contenthash].js`;
                },
                path: !dev && isNodeServer ? _path.default.join(outputPath, `chunks-${buildId}`) : outputPath,
                chunkFilename: isNodeOrEdgeCompilation ? `[name].js` : `static/chunks/[contenthash].js`,
        ...
        plugins: [
            ...
            (isNodeServer || flyingShuttle && isEdgeServer) && !dev && new (require('./webpack/plugins/next-trace-entrypoints-plugin')).TraceEntryPointsPlugin({
                rootDir: dir,
                appDir: appDir,
                pagesDir: pagesDir,
            ...
            !dev && isClient && new _csschunkingplugin.CssChunkingPlugin(config.experimental.cssChunking === 'strict'),
    ...
    const cache = {
        ...
        version: `${__dirname}|${"15.0.0-canary.145"}|${configVars}`,
