...
<Callout emoji="⚠️">
<b>Warning:</b> Make sure you place dot (.) after in the git clone command otherwise it will make a new folder named `github-repo` inside the folder `/var/www/nextjs`
</Callout>
...
- Follow [this guide](https://cloud.google.com/certificate-manager/docs/public-ca-tutorial) to get `EAB_KID` and `EAB_HMAC_KEY`
...
- View and delete the certificate
  ...
  ```
<Callout emoji="⚠️">
  <b>Warning:</b> After deleting certificate make sure to modify nginx config file (if using that deleted certificate), otherwise test will fail.
  `sudo nginx -t`
</Callout>
Planned in next post that will discuss about:

- [Caching static content and Gzip compression](/snippet/gzip-browser-cache-nginx)
- Nginx load balancer
