...
import { GitHubIcon, TwitterIcon,HomeIcon, SnippetIcon, BlogIcon, GalleryIcon, MailIcon} from '../Icons'
...
//   const [play] = useSound("../button.mp3");
  ...
  const gallery = () => {
    // play()
    window.open('https://gallery.heykapil.in', '_ blank');
  }

  const twitter = () => {
    // play()
    window.open('https://x.com/kapiljch', '_ blank');
  }
  const mail = () => {
    // play()
    window.open('mailto:contact@heykapil.com', '_ blank');
  }
  const github = () => {
    // play()
    window.open('https://github.com/heykapil', '_ blank');
  }
...
// show= showPopover , setShowPopover = useeshow
  ...
  return (
    <div className="key_nav flex">
      <div ref={targetRef} className="" onClick={() => parentFun()}><QuestionIcon className="relative h-4/5 w-4/5" /></div>
      <div className={show ? "key_nav_content show_ele" : "key_nav_content hide_ele"}>
      <div className="absolute -top-[5px] -right-[5px] bg-zinc-200 dark:bg-zinc-800 p-1 rounded-lg" onClick={handleOverlayClose}>
        ...
        <div className="elements mt-4">
          ...
          <div className="element">
            <div className="icon_element">
            <SnippetIcon className="relative h-4/5 w-4/5" aria-label='none' />
            </div>
